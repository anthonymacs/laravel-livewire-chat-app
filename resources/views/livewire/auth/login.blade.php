...
<body class="bg-gradient-to-br from-indigo-50 via-white to-purple-50 font-sans antialiased min-h-screen flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-md bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden">
        ...
        <div class="px-8 py-6">
            ...
            {{-- Error Message --}}
            @if (session('status'))
                <div class="mb-4 bg-green-50 border border-green-200 text-green-700 text-xs px-4 py-3 rounded-xl">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-4">
                ...
                {{-- Submit --}}
                <button type="submit"
                        class="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-2.5 rounded-xl text-sm font-semibold hover:opacity-90 transition shadow-md hover:shadow-lg">
                    Sign In
                </button>
